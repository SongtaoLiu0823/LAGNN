...
class LAGCN(torch.nn.Module):
    def __init__(self, concat, in_channels, hidden_channels, out_channels, num_layers,
        ...
        for _ in range(concat):
            self.convs_initial.append(GCNConv(in_channels, hidden_channels, normalize=False))

        self.convs = torch.nn.ModuleList()
        for _ in range(num_layers - 2):
            self.convs.append(
                GCNConv(concat*hidden_channels, concat*hidden_channels, normalize=False))
        self.convs.append(GCNConv(concat*hidden_channels, out_channels, normalize=False))
...
model = LAGCN(args.concat+1, data.num_features, args.hidden_channels,
              112, args.num_layers,
